...
def svc_lin_param_selection(X, y, nfolds):
    param_grid = {'C': Cs}
    grid_search = GridSearchCV(estimator = SVC(kernel='linear', probability = True, random_state = 121), param_grid = param_grid, cv = nfolds, n_jobs = n_jobs,verbose = 1)
    grid_search.fit(X, y)
    print("================================\n")
    print("SVM Linear\n")
    print("Best parameters: " + str(grid_search.best_params_))
...
def svc_poly_param_selection(X, y, nfolds):
    ...
    grid_search = GridSearchCV(estimator = SVC(kernel='poly', probability = True, random_state = 121), param_grid = param_grid, cv = nfolds, n_jobs = n_jobs,verbose = 1)
    grid_search.fit(X, y)
    print("================================\n")
...
def rf_entropy_param_selection(X, y, nfolds):
   ...
   grid_search = GridSearchCV(estimator = RandomForestClassifier(criterion = "entropy", oob_score = True, random_state = 121), param_grid = param_grid, n_jobs = n_jobs, cv = nfolds)
   grid_search.fit(X, y)
...
# Obtain prediction probabilities and evalution metrices
print("======== Train ========\n")
pred_ffnn_train_prob, ffnn_train_report = eval_metrics_ffnn(ffnn_model, X_train, Y_train, "train_ffnn")
pred_ffnn_train_prob.set_index(X_train.index, inplace=True)
...
print("======== Test ========\n")
pred_ffnn_test_prob, ffnn_test_report = eval_metrics_ffnn(ffnn_model, X_test, Y_test, "test_ffnn")
pred_ffnn_test_prob.set_index(X_test.index, inplace=True)
...
req_res = pd.DataFrame([[rf_entropy_train_report.loc['accuracy',:][1], rf_entropy_test_report.loc['accuracy',:][1]],
 [rf_gini_train_report.loc['accuracy',:][1], rf_gini_test_report.loc['accuracy',:][1]],
 [svm_lin_train_report.loc['accuracy',:][1], svm_lin_test_report.loc['accuracy',:][1]],
 [svm_poly_train_report.loc['accuracy',:][1], svm_poly_test_report.loc['accuracy',:][1]],
 [svm_rbf_train_report.loc['accuracy',:][1], svm_rbf_test_report.loc['accuracy',:][1]],
 [train_acc, test_acc]],
 columns = ["Train", "Test"], index = ["rf_entropy", "rf_gini", "svm_lin", "svm_poly", "svm_rbf", "ffnn"])
req_res.dtypes
req_res = req_res.T
...
reports = [svm_rbf_train_report, svm_rbf_test_report, svm_lin_train_report, svm_lin_test_report, svm_poly_train_report, svm_poly_test_report, rf_gini_train_report, rf_gini_test_report, rf_entropy_train_report, rf_entropy_test_report, ffnn_train_report, ffnn_test_report]
