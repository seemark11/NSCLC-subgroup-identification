...
# Merge prediction probabilities from base classifiers
...
#%%
# Merge prediction probabilities with class label
...
print("Train split")
[train_loss, train_acc] = ffnn_model.evaluate(X_ffnn_train, Y_ffnn_train_categ)
print("Validation split")
[valid_loss, valid_acc] = ffnn_model.evaluate(X_ffnn_valid, Y_ffnn_valid_categ)
...
#%%
req_res = pd.DataFrame([[log_reg_train_report.loc['accuracy',:][1], log_reg_test_report.loc['accuracy',:][1]], [train_acc, test_acc]], columns = ["Train", "Test"], index = ["log_reg", "ffnn"])
req_res.dtypes
req_res = req_res.T
req_res.to_csv(req_res_path, index = True, header = True)

#%%
reports = [log_reg_train_report, log_reg_test_report, ffnn_train_report, ffnn_test_report]
